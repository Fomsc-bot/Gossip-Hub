...
FONT_PATH = None  # optionally point to a TTF in repo for consistent branding
...
def short_title_from_text(text):
    ...
    short = " ".join(words[:3])
    # emoji selection for entertainment/gossip
    emojis = "🔥🎬"
    return f"{short} {emojis}"
...
# ---------------- Image preparation: cover & crop (no stretch) ----------------
...
def create_tts_per_line(lines, lang="en"):
    ...
    for i, line in enumerate(lines):
        ...
        dur = a.duration
        a.close()
        # minimum readable duration if TTS is very short
...
def build_final_video(bg_image_path, lines, tts_paths, durations, out_file):
    total_duration = sum(durations) + 2.0  # add end-CTA time
    log(f"Total duration (including CTA): {total_duration:.2f}s")

    # background clip (cropped image) — apply small fade and subtle zoom
    ...
    # caption clips (bottom only) — voice-synced
    caption_clips = []
    cursor = 0.0
    ...
    # add short CTA clip at the end (2 seconds)
    ...
    # combine TTS audios into one track
    audio_clips = [AudioFileClip(p) for p in tts_paths]
    combined_audio = concatenate_audioclips(audio_clips)
    # pad combined audio to total_duration (append silence for CTA)
    if combined_audio.duration < total_duration:
        # create a silent clip using AudioFileClip of tiny silent mp3 if available; fallback: set duration and let video have silence after audio end
...
def main():
    ...
    # Create punchy lines (short + attention grabbing)
